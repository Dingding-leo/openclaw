...
export const ko: TranslationMap = {
  ...
  dashboardsPage: {
    ...
    emptyDescription: "스레드를 열고 Dashboard face로 전환하여 여기에 추가하세요.",
  ...
  subtitles: {
    ...
    dashboards: "대시보드 face로 열리는 스레드입니다.",
  ...
  memoryPage: {
    ...
    engine: {
      ...
      rowTitle: "메모리 엔진",
      off: "끔",
      autoHint: "구성에 고정된 엔진이 없으므로 슬롯이 기본 소유자로 대체됩니다.",
      ...
      disabledHint:
        "메모리 슬롯이 이 플러그인을 가리키지만 플러그인 자체가 비활성화되어 있어 메모리가 실행되지 않습니다.",
      enable: "활성화",
    },
    backend: {
      title: "백엔드",
      ...
      builtin: "내장",
      qmd: "QMD",
      builtinHint: "메모리 파일은 OpenClaw가 직접 색인하고 검색합니다.",
      qmdHint: "검색이 QMD에 위임됩니다. 해당 설정은 아래에 표시됩니다.",
    },
    addons: {
      title: "애드온",
      description:
        "이 플러그인들은 슬롯을 두고 경쟁하는 대신 엔진 위에 계층으로 추가되므로 어떤 조합이든 동시에 실행할 수 있습니다.",
      activeMemory: {
        title: "활성 메모리",
      },
      memoryWiki: {
    ...
    dreaming: {
      intro:
        "Dreaming은 모든 에이전트 작업 공간에서 하나의 관리형 cron 작업으로 실행되므로 이 설정은 전역입니다. 이 설정은 {plugin} 플러그인이 소유합니다.",
      schedule: {
        title: "일정",
        description: "전체 스윕이 언제 실행되며 어떤 모델이 이를 서술하는지입니다.",
      },
      frequency: {
        label: "Dreaming 빈도",
        help: "전체 드리밍 스윕(라이트, REM, 딥)의 Cron 주기입니다. 플러그인 기본값을 사용하려면 비워 두세요.",
        placeholder: "0 3 * * *",
      },
      timezone: {
        label: "시간대",
      ...
      model: {
        label: "드리밍 모델",
        help: "꿈 일기 서술을 위한 공급자/모델 재정의입니다. 서브에이전트 모델 재정의가 허용되어야 합니다.",
      ...
      storage: {
        ...
        separateReportsHelp: "드리밍 보고서를 기본 메모리 파일에서 분리하여 보관합니다.",
      },
      phases: {
        light: {
          title: "라이트 단계",
          description: "재생 후보를 준비하는 저비용 최근 활동 검사입니다.",
        },
        deep: {
          title: "Deep 단계",
          description: "단기 항목을 메모리로 승격시키는 점수 기반 승격 검사입니다.",
        },
        rem: {
          title: "REM 단계",
          description: "룩백 기간 전반에서 반복되는 테마를 찾는 패턴 검사입니다.",
        },
      },
      phaseFields: {
        enabled: "활성화됨",
        ...
        minUniqueQueriesHelp: "항목을 노출시킨 서로 다른 쿼리의 수입니다.",
        recencyHalfLifeDays: "최신성 반감기(일)",
        recencyHalfLifeDaysHelp: "오래된 회상 신호가 얼마나 빨리 가중치를 잃는지 지정합니다.",
        maxAgeDays: "최대 기간(일)",
  ...
  labsPage: {
    ...
    localModelLean: {
      title: "로컬 모델용 경량 도구",
      description:
        "작은 로컬 모델이 잘 처리하지 못하는 무거운 기본 도구를 제외하여, 안정적으로 사용할 수 있는 짧은 세트만 남깁니다.",
    },
    auditMessages: {
      title: "메시지 감사 메타데이터",
      description:
  ...
  dreaming: {
    ...
    toggleConfirmation: {
      subtitle: "드리밍은 전역 설정이며, 이 에이전트에만 적용되지 않습니다.",
      enableTitle: "모든 에이전트에 대해 드리밍 켜기",
      enableDetail:
        "야간 드리밍 스윕이 구성된 모든 에이전트 작업 공간에서 실행되어 단기 회상을 장기 기억으로 승격합니다. 이 설정은 즉시 적용됩니다.",
      enableConfirm: "드리밍 켜기",
      disableTitle: "모든 에이전트에 대해 드리밍 끄기",
      disableDetail:
        "야간 드리밍 스윕이 이 에이전트뿐 아니라 구성된 모든 에이전트에 대해 중지됩니다. 이미 기록된 기억은 유지되며, 새로 승격되는 항목은 없습니다. 이 설정은 즉시 적용됩니다.",
  ...
  chat: {
    ...
    sessionRoute: {
      chooseTitle: "세션 선택",
      multipleMatches: "{shortId}과(와) 일치하는 세션이 두 개 이상입니다.",
