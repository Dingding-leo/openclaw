...
export const ja_JP: TranslationMap = {
  ...
  dashboardsPage: {
    ...
    emptyDescription: "スレッドを開き、Dashboard 表示に切り替えてここに追加します。",
  ...
  memoryPage: {
    ...
    engine: {
      title: "エンジン",
      description:
        "メモリスロットを所有するメモリプラグインは常に1つだけです。エンジンを選択すると、それが有効になり他が無効になります。",
      rowTitle: "メモリエンジン",
      off: "オフ",
      autoHint:
      ...
      offHint:
        "メモリは設定でオフになっています: plugins.slots.memory が none に設定されています。",
    ...
    backend: {
      ...
      builtin: "組み込み",
      qmd: "QMD",
      builtinHint: "メモリファイルは OpenClaw 自身によってインデックス化され、検索されます。",
      qmdHint: "取得は QMD に委任されます。設定は以下に表示されます。",
    },
    addons: {
      title: "アドオン",
      description:
        "これらのプラグインはスロットを奪い合うのではなくエンジンの上に重なるため、任意の組み合わせを同時に実行できます。",
      activeMemory: {
        title: "アクティブメモリ",
      },
      memoryWiki: {
    ...
    search: {
      intro:
        "メモリのオーバーライドがないすべてのエージェントで共有される埋め込みと取得のデフォルト設定。",
    },
    dreaming: {
      intro:
        "ドリーミングはすべてのエージェントワークスペースにわたって1つの管理された cron ジョブとして実行されるため、これらの設定はグローバルです。これらは {plugin} プラグインが所有します。",
      schedule: {
        title: "スケジュール",
      ...
      frequency: {
        label: "ドリーミングの頻度",
        help: "完全なドリーミングスイープ（ライト、REM、その後ディープ）のcronスケジュール。プラグインのデフォルトを使用する場合は空のままにします。",
        placeholder: "0 3 * * *",
      },
      timezone: {
        label: "タイムゾーン",
        help: "cronスケジュールの解釈に使用するIANAタイムゾーン。",
      ...
      verboseLogging: {
        label: "詳細ログ",
        help: "各ドリーミングフェーズを詳細にログ記録します。しきい値を調整する際に便利です。",
      },
      storage: {
        title: "ストレージ",
        description: "昇格されたメモリとドリーミングレポートの書き込み先。",
        modeLabel: "ストレージモード",
        modeHelp:
          "インラインはメモリファイルに書き込みます。separateは専用のレポートファイルを保持します。",
        modes: {
          inline: "インライン",
          separate: "分離",
          both: "両方",
      ...
      phases: {
        light: {
          title: "ライトフェーズ",
          description: "リプレイ候補をステージングする低コストな直近アクティビティのパスです。",
        },
        deep: {
          title: "ディープフェーズ",
          description: "短期エントリをメモリへ昇格させるスコア付きの昇格パスです。",
        },
        rem: {
          title: "REMフェーズ",
          description: "参照期間全体で繰り返し現れるテーマを探すパターンパスです。",
        },
      },
      phaseFields: {
        enabled: "有効",
        ...
        lookbackDays: "参照日数",
        lookbackDaysHelp:
          "このフェーズがどこまで過去を読み取るか。プラグインのデフォルトを使う場合は空欄のままにします。",
        limit: "上限",
        limitHelp: "このフェーズが1回の実行で処理するエントリの最大数です。",
        dedupeSimilarity: "重複判定の類似度",
        dedupeSimilarityHelp: "この値を超える類似度の2つの候補は重複として扱われます。",
        minScore: "最小スコア",
        minScoreHelp: "エントリが昇格するために達する必要のあるスコアです。",
        minRecallCount: "最小リコール回数",
        minRecallCountHelp: "エントリが昇格できるまでに呼び出される必要のある回数です。",
        minUniqueQueries: "最小ユニーククエリ数",
        minUniqueQueriesHelp: "エントリを表示した個別のクエリが何件必要かを指定します。",
        recencyHalfLifeDays: "直近性の半減期（日）",
        ...
        maxPromotedSnippetTokensHelp:
          "昇格された各スニペットのトークン予算です。来歴情報は引き続き付随します。",
        minPatternStrength: "最小パターン強度",
        minPatternStrengthHelp:
          "繰り返し発生するパターンが報告されるために達する必要のある強度です。",
      },
      agentScope: {
        title: "エージェントビュー",
        description:
          "上記の設定はグローバルです。以下のドリームダイアリー、短期カウント、メンテナンスアクションは1つのエージェントに属します。",
        rowTitle: "エージェント",
      },
      unsupported: {
        title: "ドリーミング設定",
        ...
        description:
          "{plugin} プラグインがメモリスロットを所有しており、その設定スキーマにドリーミングセクションがないため、これらの設定は保存できません。編集するには、Overviewタブでエンジンを切り替えてください。",
      },
    },
  },
  sessionsPage: {
  ...
  labsPage: {
    ...
    toolSearch: {
      title: "ツール検索",
      description:
        "限定されたツールディレクトリを常に表示し、残りは検索の背後に遅延させることで、大規模なMCPやプラグインのカタログがプロンプトを圧迫しないようにします。",
    },
    localModelLean: {
      title: "ローカルモデル向けの軽量ツール",
      description:
        "小規模なローカルモデルが苦手とする重量級のデフォルトツールを外し、確実に使用できる短いセットを残します。",
    },
    auditMessages: {
      title: "メッセージ監査メタデータ",
      description:
        "ダイレクト会話のコンテンツを含まないメタデータを監査台帳に記録します。メッセージの内容は保存されません。",
    },
  },
  aboutPage: {
  ...
  dreaming: {
    ...
    toggleConfirmation: {
      subtitle: "Dreamingはグローバル設定です。このエージェントに限定されるものではありません。",
      enableTitle: "すべてのエージェントでDreamingをオンにする",
      enableDetail:
        "夜間のDreamingスイープが、設定済みのすべてのエージェントワークスペースで実行され、短期リコールを長期メモリへ昇格させます。これはすぐに適用されます。",
      enableConfirm: "Dreamingをオンにする",
      disableTitle: "すべてのエージェントでDreamingをオフにする",
      disableDetail:
        "夜間のDreamingスイープが、このエージェントだけでなく、設定済みのすべてのエージェントで停止します。すでに書き込まれたメモリは残りますが、新しいものは昇格されません。これはすぐに適用されます。",
  ...
  chat: {
    ...
    sessionRoute: {
      ...
      additionalMatches: "検索結果が残っています。より長いidプレフィックスを使用してください。",
    },
    sessionSharing: {
      menu: "スレッドの共有",
